...
- **Adding or changing a `permissions:` block?** Trace whether each scope is actually consumed by the default `GITHUB_TOKEN`, not assumed. A step that passes its own `github_token:`/`GH_TOKEN:` (e.g. the `ocs-agent` app token) never touches the job's `permissions:`-governed default token for that call.
