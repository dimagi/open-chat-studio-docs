# Documentation Self-Review Checklist

- [ ] Page is no more than ~100 lines of markdown source, or split into multiple pages if longer.
- [ ] Content that belongs to a different page type (see `page-type-contract.md`) is separated into its own page rather than mixed in.
- [ ] No duplication with other existing pages — merged or linked instead.
- [ ] Page opens with a purpose statement: what it covers and why it matters.
- [ ] All OCS terminology and technical terms are defined on first use or linked to a Concept page.
- [ ] Enough internal links to and from related content.
- [ ] Use "## See also" for a list of related reading links. Use "## Next steps" when the section points the reader to the next page in a sequence.
- [ ] Semantic linefeeds (one sentence per line) are used in newly written or edited paragraphs, not line breaks at a fixed column width.
- [ ] Sentences are under 25 words.
- [ ] No mannered prose — no metaphor or flourish where a literal phrase says it directly ("a dial worth turning" → "a parameter worth varying").
- [ ] Admonitions (`!!! note`, `!!! warning`) are used only for genuine notes, warnings, or tips — not as a formatting crutch.
- [ ] "Chatbot", not "bot" — except where the text quotes a UI string, code identifier, or product name verbatim (e.g. **New Bot Message**, `bot_message`, Multi-bot, Telegram bot).
- [ ] "Participant" for the person chatting with a chatbot; "user" for a team member using OCS — except where the text quotes a UI string verbatim (e.g. **Prompt the bot to message the user**); example prompts, code, and code comments keep their own wording.
- [ ] Names of UI controls the reader clicks or selects are bold. Keep quotes for literal error messages.
