...
# Changelog
## Mar 10, 2026
* **CHANGE** The Send Email pipeline node's subject and recipient fields now support Python format string variables (e.g. `{participant_id}`, `{session_id}`), and a new optional body field accepts Jinja2 templates — the same variables available in the Render Template node. Existing pipelines are unaffected.
