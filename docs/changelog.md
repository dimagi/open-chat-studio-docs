...
* **BUG** Fixed a `KeyError` raised when a chatbot prompt contained the `{source_material}` placeholder but no source material was available (for example, when previewing a prompt with an unset or invalid source material). The placeholder now always resolves, substituting an empty string when no source material is present.
