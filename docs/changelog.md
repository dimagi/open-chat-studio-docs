...
# Changelog
## Apr 21, 2026
* **NEW** Annotation exports (CSV and JSONL) now include three additional fields: `session_external_id` (the external UUID of the linked session), `flagged` (whether the item is flagged), and `flagged_reason` (the list of flag entries). Flagged items with no annotations are also included in the export.
