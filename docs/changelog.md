...
* **NEW** A participant's data can now be edited directly on a chatbot session's **Participant Data** tab, which was previously read-only. The JSON is validated as you type, and **Format**, **Revert** and **Save** actions sit above it.
* **NEW** The **Export** button on a chatbot session's **Messages** tab now downloads that single session's messages as a CSV, where before it was a disabled placeholder. It needs the same permission as the existing bulk chat export.
* **CHANGE** The chatbot session detail page has been redesigned. A compact header carries the session's status, chatbot, participant, start date, message count, usage, duration and tags, while the Session ID, Remote ID, ended time, platform, annotation queues and comments have moved behind a **Session details** toggle. Messages now read as chat bubbles, with the per-message trace, copy-link, tag and annotation actions revealed on hover, and the **Participant Schedules** and **Chatbot Events** tabs are now tables whose rows open their details in a modal. **New Session** and **End Session** are always shown, disabled with a tooltip explaining why when they don't apply to the session.
...
* **BUG** Fixed saving a participant's data writing to a record the chatbot never reads when that chatbot is running a published version, so the edit had no effect on the live conversation.
