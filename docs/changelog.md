...
## Sep 16, 2026
* **NEW** A chat session started through an [OAuth-mode](chat_widget/reference.md#oauth-credential-mode) Chat Widget & API channel can now be renewed instead of restarted. The new `POST /api/chat/{session_id}/token/` endpoint issues a fresh session token for an existing session, and both session start and renewal responses now say when the token expires, so a conversation continues past the token's lifetime rather than starting over. Renewal needs a valid OAuth token for the channel, is throttled per OAuth application so one busy visitor cannot starve the others, and refuses anything else — including an unknown session — with the same `401`. See the [Chat API reference](api/v1/index.md).
* **CHANGE** A Chat API session token now expires a fixed time after it was issued — five minutes by default, or the channel's **Session token lifetime** — rather than being measured from when the session was created. Re-issuing a token renews access, and tokens issued before this release keep the previous behaviour.
* **CHANGE** An OAuth access token requested with the `chat:start` scope **alone** now expires after 60 seconds instead of 10 hours, because such a token is meant to be handed to a browser to start a single session. A token that also carries other scopes keeps the long lifetime, so request `chat:start` on its own for browser-bound tokens. See [Getting started with OAuth2](api/getting_started_with_oauth.md#tokens-for-starting-chat-sessions).
* **CHANGE** Self-hosted deployments can now set two optional environment variables to control these lifetimes: `OAUTH_ACCESS_TOKEN_EXPIRE_SECONDS` (default `36000`) and `OAUTH_CHAT_START_TOKEN_EXPIRE_SECONDS` (default `60`).
