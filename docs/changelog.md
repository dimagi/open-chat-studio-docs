...
## Sep 21, 2026
* **NEW** The [v2 chatbots API](api/v2/chatbots.txt) can now publish and archive a chatbot. One endpoint publishes a new version of the working draft, taking an optional `version_description`; a second reports the chatbot's publish state — `pending` while a publish is still running, or `completed` with the newest `version_number` once it has finished — so a client can poll for the outcome; and a third archives the chatbot.
A publish is refused with a `422` listing `pipeline_errors` when the draft pipeline is not valid, and with a `409` when a publish or revert is already under way. Archiving is refused with a `409` listing the channels still attached to the chatbot, each with its id, platform and name, so you can find and remove them in the web app first.
...
* **BUG** An [LLM evaluator](concepts/evaluations/evaluators.md) whose model declines to return structured output — it refuses, the provider blocks the reply, or it does not call the tool — now reports `The model did not return structured output:` followed by what the model said, or the provider's stop reason if it said nothing, in the same `error (<evaluator name>)` column. Previously the results grid showed `'NoneType' object has no attribute 'model_dump'`. A schema mismatch is still retried; a declined request is not, since asking again gets the same answer.
...
## Sep 16, 2026
* **NEW** Chatbot [events](concepts/events.md) have a third trigger type: a **Scheduled Trigger** that fires once at a date, time and timezone you pick, using the **Create Scheduled Event** button on a chatbot's **Events** tab. It runs the same actions as the existing triggers — end the conversation, prompt the bot to message the user, trigger a schedule or start a pipeline — against the chatbot's most recent session, and records each attempt in the event logs. Only published chatbots fire, and the trigger's date and time must be in the future.
...
## Sep 14, 2026
* **NEW** The [pipeline](concepts/pipelines/index.md) builder now has undo and redo. Use the two new buttons next to the zoom and fit-view controls, or press `Ctrl+Z` to undo and `Ctrl+Shift+Z` (or `Ctrl+Y`) to redo. Adding, deleting, moving and connecting nodes, and changing a node's parameters or label, can all be stepped back and forward. Dragging a node counts as one step, and deleting a node undoes together with the edges that were removed with it. Undo and redo are saved like any other edit, so an undone change survives a page reload, and neither is available on a pipeline you can only view.
...
## Sep 8, 2026
* **NEW** Team administrators can require two-factor authentication for their team with the new **Require two-factor authentication** checkbox under **Members & access** on the team settings page. Once it is on, every member of that team — including the administrator who enabled it — must set up two-factor authentication before they can use Open Chat Studio, on any page and not just that team's. Members of teams that have not enabled it are unaffected. The setting is off by default.
...
* **CHANGE** [Team Settings](concepts/team/index.md) has been redesigned as a single page with four sections — **Integrations**, **Members**, **Developer** and **Data**. Every service provider (LLM, speech, messaging, authentication, tracing) is now a row in one [Integrations](concepts/team/integrations.md) table you filter by category, reached from a single grouped **Add integration** button in place of the separate per-category pages. Active members and pending invitations share one searchable, filterable [Members](concepts/team/members.md) table, the team name is edited in place, and one **Save key** button in the **Data** section now saves the migration public key and migration mode together. No provider, member or invitation needs reconfiguring.
* **BUG** An invitation that fails validation now keeps the **Invite people** dialog open with the error shown. Previously the form closed with no invitation sent and no explanation.
...
* **BUG** A configuration mistake in a [Send an email](concepts/pipelines/nodes.md#send-an-email-node) or [Render a template](concepts/pipelines/nodes.md#render-a-template-node) node — a template referring to a variable that doesn't exist, or an invalid email address — is now treated as a configuration error rather than a system failure, and the participant receives a generic error response.
...
* **NEW** **LiteLLM** can now be added as an LLM provider, for teams running their own OpenAI-compatible model gateway (a self-hosted or third-party LiteLLM proxy). Setup is the same as any other provider except that you also give the proxy's **Base URL**; no models are pre-configured, so add the backend models your proxy serves yourself. See [LLM Service Providers](concepts/team/llm_providers.md) and [Add a Custom LLM Model](how-to/add_custom_llm_model.md).
