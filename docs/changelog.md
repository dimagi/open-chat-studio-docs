...
## Aug 20, 2026
* **CHANGE** Turning off a channel's **Enabled** toggle now blocks the channel completely. Previously a disabled channel still allowed new conversations to be started (from the chat widget, the public web chat link, Slack, or the chatbot management pages) and still sent out scheduled messages, event action messages and API-triggered messages — only the bot's replies to incoming messages were stopped. New sessions on a disabled channel are now refused, and bot-initiated messages to a disabled channel are no longer sent. See [Disabling a channel](concepts/channels.md#disabling-a-channel).
