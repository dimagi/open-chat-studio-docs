...
## Sep 18, 2026
* **NEW** **OpenRouter** can now be added as an [LLM provider](concepts/team/llm_providers.md). OpenRouter routes requests to models from OpenAI, Anthropic, Meta, Google, DeepSeek and others through a single API key, so one provider entry gives your chatbots access to all of them. A few models are pre-configured, with `openai/gpt-4.1-mini` as the default; add any others as [custom models](how-to/add_custom_llm_model.md).
