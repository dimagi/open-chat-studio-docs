...
Metrics are collected across all LLM nodes in the pipeline (RouterNode and LLMResponseWithPrompt). Pipelines that have no LLM nodes display "—" for all metrics. Token counts are provider-reported values; pipelines that fail before LLM calls are completed may show partial or null values.
