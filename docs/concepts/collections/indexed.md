...
To open the sync log for a document source:

1. Navigate to the **Collections** page and open the relevant collection.
2. Locate the document source whose history you want to review.
3. Click the **View Sync Logs** button in the document source header.

A modal opens showing the sync history for that source. Each entry in the log includes:

| Column    | Description                                                   |
|-----------|---------------------------------------------------------------|
| Timestamp | When the sync ran                                             |
| Status    | Outcome of the sync: **Success**, **Error**, or **In Progress** |
| Added     | Number of files added during the sync                         |
| Updated   | Number of files updated during the sync                       |
| Removed   | Number of files removed during the sync                       |
| Duration  | How long the sync took to complete                            |

For failed syncs, click the error entry to view the full error message in an inline modal.

Use the **Show errors only** checkbox at the top of the modal to filter the list to failed syncs. This is useful when you want to review a long history and focus only on runs that require attention.

!!! note
    The sync log uses pagination. Click the navigation controls at the bottom of the modal to move through older entries.
