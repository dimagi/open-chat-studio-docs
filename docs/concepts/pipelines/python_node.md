...
It provides the following:
 ...
 - ability to call [external APIs](../../tech-hub/external-api-calls/http_client.md)
...
The Python node provides an `http` global that enables secure HTTP requests to external APIs. See the [HTTP Client](../../tech-hub/external-api-calls/http_client.md) page for full documentation.
...
In addition to these keys, the temporary state can also contain custom key-value pairs that can be set and accessed by the Python node and by the [Static Router](./router_nodes.md#static-router-node) node.
