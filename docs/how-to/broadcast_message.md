...
Use it to share an announcement, a service update, or a reminder with everyone
who has ever talked to the bot on a given channel.

## Prerequisites

- You need the **Can Invite Participants** permission on the team. This is the
  same permission required to invite participants to a chatbot — see the
  [permissions table][groups] for which roles have it. If you don't have it,
  ask your team admin to grant it or to send the broadcast for you.
- The chatbot needs at least one participant-facing channel connected — for
  example Telegram, WhatsApp, Slack, or Email. See
  [Deploying your chatbot to different channels][deploy] if none are set up yet.

## Send a broadcast

1. Open the chatbot and go to its home page.
...
The **Send** button stays disabled until you've selected at least one channel
and entered a message, so you can't send an empty broadcast by mistake.

Sending queues the broadcast for delivery in the background, so it may take a
short while to reach everyone after you click **Send** — the dialog closes
immediately and doesn't wait for delivery to finish.
...
templates use — shown by the live counter in the dialog. This keeps the
message short enough to deliver on every channel you've selected, not just
the most permissive one.

Before sending, OCS also collapses line breaks and repeated spaces in your
...
channel you picked. The dialog warns you about this before you send, so write
your message expecting it to arrive as one paragraph rather than relying on
...
Meta. This is because a broadcast isn't a reply to something the participant
just sent — it can go out at any time, including outside WhatsApp's 24-hour
customer-service window, and WhatsApp only allows messages outside that
window if they use an approved template.
