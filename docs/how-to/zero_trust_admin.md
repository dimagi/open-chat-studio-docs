# Zero Trust Administration Guide

This guide is for NGO administrators and technical staff who manage a self-hosted Open Chat Studio instance and want to enable secure remote access for their team using Cloudflare Zero Trust.

!!! note "Self-hosted deployments only with the Zero Trust Access on cloudflare"
    This guide applies only to self-hosted Open Chat Studio instances. If you are using the hosted version at openchatstudio.com or version that uses reverse proxy, this does not apply to your setup.

---

## What this does

Cloudflare Zero Trust allows your team to access Open Chat Studio securely from anywhere; office, home, or another country, without exposing a public domain or opening firewall ports.

Every connection is verified before it reaches your server:

- The device must be enrolled and registered
- The user must be assigned to the application
- A one-time code must be confirmed in the browser (once per working day)
- An active session must be within the allowed time window

External messaging providers (Twilio, WhatsApp, Telegram, Slack) continue to deliver messages normally. Their webhook endpoints are handled separately and do not require WARP.
...
- Your Open Chat Studio server running with Docker

For the technical setup: Docker, tunnel token, and tunnel ingress configuration, see the [developer setup guide](./cloudflare_tunnel.md). This guide covers the ongoing administration: managing users, devices, sessions, and logs.
...
### Step 4: Send them the setup guide

Ask them to install the [Cloudflare WARP client](https://developers.cloudflare.com/cloudflare-one/connections/connect-devices/warp/download-warp/), then follow these steps to connect:

1. Open WARP → **Preferences → Account → Login with Cloudflare Zero Trust**
2. Enter the organisation team name (ask your admin if you don't have it)
3. Enter the OTP code sent to their work email
4. Turn WARP on - the icon should show **Connected**
5. Open a browser and go to the app hostname (e.g. `https://ocs.your-org.com`)
...
This terminates their Access token immediately rather than waiting for it to expire naturally.
...
When a team member gets a new laptop or their device is replaced:
...
Their email assignment in the Access policy does not change, only the device registration updates. Access is restored immediately once the new serial is saved.
...
!!! warning "Keep WARP session longer than app session"
    The WARP session must always be set equal to or longer than the app session. If WARP expires before the app session, users will be disconnected from the network while the application still considers them logged in. This causes confusing errors.

---

## Reading audit logs

Cloudflare keeps two separate logs.
...
- Check whether a removed user tried to access after revocation

### Gateway logs - network activity

**Location:** Zero Trust → Logs → Gateway

Each entry shows which private addresses were reached, how much data was transferred, and which device made the request. Use this log to monitor usage or investigate unusual activity.
...
If all of the above are correct, check the Access logs. The deny reason in the log will identify exactly which check is failing.
...
The tunnel runs with auto-restart, most crashes recover within seconds. If it has been unavailable for more than a minute:

1. Check the tunnel status at **Networks → Tunnels**.
2. If the status shows **Unhealthy** or **Inactive**, contact your technical administrator to restart the `cloudflared` Docker container.
...
4. If the problem continues, remove their email from the policy and re-add it to reset their Access session.
...
- [Developer setup guide](./cloudflare_tunnel.md) - Docker, tunnel token, and ingress configuration
- [Zero Trust Access overview](../concepts/zero_trust_access.md) - tool comparison and webhook path requirements
