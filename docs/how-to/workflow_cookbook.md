...
Router nodes can have unconnected outputs as [seen above](#safety-check-in-parallel), enabling more flexible routing patterns where not all paths need to be explicitly handled.

You can also connect multiple router outputs to the same input of another node. This can be useful if you want the [router node][router_nodes] to categorize the input without affecting the execution flow.
...
You might use this to perform some logic in the [**Python node**][python]:
...
Then in the LLM node prompt you can use the [**temp_state**][prompt_vars] to inject the category:
...
2. **Create a Python Node**: Use a [Python node][python] to read and process the uploaded file contents from the [temporary state][temp_state] - specifically from the [attachments][attachments] key.
